use super::{LuaValue, LuaState, Result, eval_expr, boolean_coercion, num_coercion};
...
mod tests {
    ...
    fn test_math_negation() {
        ...
        // 1. + -1. == 0.
        let res = eval_unary_expr(&Box::new(Exp::Num(Numeral::Float(1.0))), &UnOp::Minus, &ctx).unwrap();
        assert_eq!(res, LuaValue::Float(-1.));

        let res = eval_unary_expr(&Box::new(Exp::Num(Numeral::Int(0))), &UnOp::Minus, &ctx).unwrap();
        assert_eq!(res, LuaValue::Integer(0));

        let res = eval_unary_expr(&Box::new(Exp::Num(Numeral::Int(-4))), &UnOp::Minus, &ctx).unwrap();
        assert_eq!(res, LuaValue::Integer(4));

        let res = eval_unary_expr(&Box::new(Exp::Str(StringLit(Cow::from(&b"5.5"[..])))), &UnOp::Minus, &ctx).unwrap();
    ...
    fn test_bool_negation() {
        let ctx = LuaState::new();
        // 1. + -1. == 0.
        let res = eval_unary_expr(&Box::new(Exp::Num(Numeral::Float(1.0))), &UnOp::BoolNot, &ctx).unwrap();
        assert_eq!(res, LuaValue::Boolean(false));

        let res = eval_unary_expr(&Box::new(Exp::Num(Numeral::Int(0))), &UnOp::BoolNot, &ctx).unwrap();
        assert_eq!(res, LuaValue::Boolean(false));

        let res = eval_unary_expr(&Box::new(Exp::Str(StringLit(Cow::from(&b""[..])))), &UnOp::BoolNot, &ctx).unwrap();
        assert_eq!(res, LuaValue::Boolean(false));
    ...
    fn test_bool_length() {
        let ctx = LuaState::new();
        let res = eval_unary_expr(&Box::new(Exp::Str(StringLit(Cow::from(&b"1234"[..])))), &UnOp::Length, &ctx).unwrap();
        assert_eq!(res, LuaValue::Integer(4));
    ...
    fn test_bitwise_inversion() {
        let ctx = LuaState::new();
        let res = eval_unary_expr(&Box::new(Exp::Num(Numeral::Int(0))), &UnOp::BitNot, &ctx).unwrap();
